...
public class TollCalculator
{
    ...
    public int GetTotalTollFee(Vehicle vehicle, DateTime[] dates)
    {
        int totalFee = 0;
        int nextFee = 0;        
        int HourHighestFee = GetTollFee(dates[0], vehicle);
        totalFee = HourHighestFee;
        ...
        foreach (DateTime date in dates.Skip(1)) {
            ...
            if (timer.withinLastHour() == true) {
                if(nextFee > HourHighestFee) {
                    totalFee +=  nextFee - HourHighestFee;
